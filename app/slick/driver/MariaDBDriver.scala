...
trait MariaDBDriver extends JdbcDriver { driver =>
  ...
  override protected def computeQueryCompiler = super.computeQueryCompiler.replace(new MariaDBResolveZipJoins) - Phase.fixRowNumberOrdering
  ...
  class MariaDBResolveZipJoins extends ResolveZipJoins {
  ...
  class SequenceDDLBuilder[T](seq: Sequence[T]) extends super.SequenceDDLBuilder(seq) {
    override def buildDDL: DDL = {
      ...
      if(!seq._cycle && (seq._minValue.isDefined && desc || seq._maxValue.isDefined && !desc))
        throw new SlickException("Sequences with limited size and without CYCLE are not supported by MariaDBDriver's sequence emulation")
...
object MariaDBDriver extends MariaDBDriver
